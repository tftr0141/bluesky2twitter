function addDataRow(data) {
  ...
  return updateMySheet(sheetData, `A1:${alphabets[lastColumn - 1]}${sheetData.length}`);
}
...
function getMySheet(_ranges = SHEET_NAME) {   // all range for default
  let response = {};
  try {
    // SheetsAPIを呼び出し
    ...
    // console.log(e);
    throw new Error('Getting sheet failed. error: \n' + e.message + '\n responce: \n' + response.getContentText());
    return;
  }
}
